...
let parseUser = (json: JSON.t): result<user, S.error> => {
  json->S.parseWith(userSchema)
}

let serializeUser = (value: user): JSON.t => {
  value->S.serializeWith(userSchema)->Result.getExn
}
----
